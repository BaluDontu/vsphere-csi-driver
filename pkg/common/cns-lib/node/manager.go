...
type Manager interface {
	...
	// SetUseK8sCSINodeObj sets whether the node manager should use
	// K8s CSINode API object or the K8s Node API object to retrieve
	// the node UUID.
	SetUseK8sCSINodeObj(useK8sCSINodeObj bool)
...
type defaultManager struct {
	...
	// useK8sCSINodeObj uses K8s CSINode API instead of
	// K8s Node to retrieve the node UUID.
	useK8sCSINodeObj bool
}
...
// K8s CSINode API object or the K8s Node API object to retrieve
// node UUID.
func (m *defaultManager) SetUseK8sCSINodeObj(useK8sCSINodeObj bool) {
	m.useK8sCSINodeObj = useK8sCSINodeObj
}
	...
	m.nodeNameToUUID.Range(func(nodeName, nodeUUID interface{}) bool {
		if nodeName != nil && nodeUUID != nil && nodeUUID.(string) == "" {
			...
			k8snodeUUID, err := k8s.GetNodeUUID(ctx, m.k8sClient,
				nodeName.(string), m.useK8sCSINodeObj)
			if err != nil {
				log.Errorf("failed to get node UUID from node: %q. Err: %v", nodeName, err)
				return true
			}
